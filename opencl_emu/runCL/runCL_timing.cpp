...
int b_cleanup = 0;
    ...
    b_cleanup = (int)( m_nbr_args != _nbr_args || m_nbr_dim != _nbr_dim);
...
cl_ulong usedLocalMemory;
			...
			if ( (m_args[i].m_extarg.m_flags & CL_ARG_VALUE) == CL_ARG_VALUE )
			{
			...
			else if ( (m_args[i].m_extarg.m_flags & CL_ARG_LCL_MEM_SZ) == CL_ARG_LCL_MEM_SZ )
			{
       			    memcpy(&m_args[i].m_extarg, &_args[i], sizeof(ClKrnlArg));
                    m_args[i].m_resource = 0;
...
// send one of device info
			...
			else if ( (m_args[i].m_extarg.m_flags & CL_ARG_BUFFER_PTR) == CL_ARG_BUFFER_PTR )
			{

				if ( m_args[i].m_extarg.m_arg.parg != m_args[i].m_extarg.m_arg.parg )
				{
					 releaseBuf(&m_args[i]);
// release buffer
                     m_args[i].m_extarg.m_flags = CL_ARG_NOT_INIT;
...
// populate input
    ...
    for(int i = 0; i < m_nbr_args; i++)
	{
		...
		if ( ((m_args[i].m_extarg.m_flags & CL_ARG_BUFFER_PTR) == CL_ARG_BUFFER_PTR)
			&& (((m_args[i].m_extarg.m_flags & CL_ARG_INPUT_PTR) == CL_ARG_INPUT_PTR) || ((m_args[i].m_extarg.m_flags & CL_ARG_INPUTOUTPUT_PTR) == CL_ARG_INPUTOUTPUT_PTR))
			&& ((m_args[i].m_extarg.m_flags & CL_ARG_POPULATE_PTR) == CL_ARG_POPULATE_PTR)
			)
		{
    /* Enqueue write  */
...
// read results
    ...
    for(int i = 0; i < m_nbr_args; i++)
	{
		...
		if ( ((m_args[i].m_extarg.m_flags & CL_ARG_BUFFER_PTR) == CL_ARG_BUFFER_PTR)
			&& (((m_args[i].m_extarg.m_flags & CL_ARG_OUTPUT_PTR) == CL_ARG_OUTPUT_PTR)
			|| ((m_args[i].m_extarg.m_flags & CL_ARG_INPUTOUTPUT_PTR) == CL_ARG_INPUTOUTPUT_PTR))
		   )
		{
...
int 
ClRunnable::FindProgram(cl_program _program, ClRunnableProgram** _runnable_program )
{
cl_int status = SDK_FAILURE;
	if ( m_Programs )
	{
...
int 
ClRunnable::FindProgram(const char* _program_location, ClRunnableProgram** _runnable_program)
{
cl_int status = SDK_FAILURE;
	if ( m_Programs )
	{
		for( int i = 0; i < m_nbr_programs; i++)
		{
			if ( !strcmp(m_Programs[i].m_program_loc,_program_location) )
			{
			   *_runnable_program = &m_Programs[i];
			   status = CL_SUCCESS;
			   break;
...
int ClRunnable::AddProgram(const char* _program_location, cl_program _program)
{
 cl_int status = SDK_FAILURE;
 	if ( m_Programs && m_nbr_programs < m_max_programs)
	{

		strcpy_s(m_Programs[m_nbr_programs].m_program_loc,_MAX_PROGRAM_NAME, _program_location);
...
ClRunnable::buildProgramCL(const char* _program_location)
{
 ...
 cl_program program;
	if ( FindProgram(_program_location, &run_program ) == CL_SUCCESS )
	{
        return(status);
	}
...
//    kernelFile.open("FaceRecogn_Kernels.cl");
    kernelFile.open(_program_location);
    ...
    status = AddProgram(_program_location, program);
	return(status);
}
...
static int sPass = 0;
int 
ClRunnable::runInstKernelCL(const char* _device_name, const char * _kernel_location, const char *_kernel_entry_name, size_t _nbr_dim, size_t _domainSize[3], size_t _groupSize[3], int _nbr_args, ClKrnlArg *_args)
{
 ...
 double kernelTime;
	...
	if ( _kernel_location )
	{
	     status = buildProgramCL(_kernel_location);

		 if ( status == SDK_SUCCESS)
		 {
            FindProgram(_kernel_location, &run_program);
	   ...
	   if ( run_kernel )
	   {
        ...
        /* Compute kernel time */
               ...
               printf("N: %d, Time(ms): %lf\n",
                          ++sPass, 
                          sAccumTime / (iterations*sPass * 1000));
...
size_t localThreads[3] = {1, 1, 1};
        ...
        ret = sRunnables[0]->runInstKernelCL(0,_program,_kernel_entry_name,_domainDim, globalThreads, localThreads, count, _args);
