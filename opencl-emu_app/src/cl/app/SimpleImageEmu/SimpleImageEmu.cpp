...
//	if (memcmp(verificationOutput,outputImageData2D, width * height * pixelSize))
	{
		for(int j = 0; j < (int)height; j++)
		{
           for(int i = 0; i < (int)width; i++)
		   {
			   if ( memcmp(&outputImageData2D[j*width + i], &((cl_char4*)verificationOutput)[j*width + i], sizeof(cl_char4)) )
			   {
				   printf("%d %d\n", i, j);
                   sampleCommon->error("Failed to verify output image!");
                   return SDK_FAILURE;
			   }
		   }
		}
...
ClImageArg img0, img1;
        memset(&img0, 0, sizeof(ClImageArg));
        memset(&img1, 0, sizeof(ClImageArg));
        img0.image_format = imageFormat;
		img0.flags = CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR;
		img0.image_width = width;
		img0.image_height = height;
        img0.host_ptr = inputImageData;
        img1.image_format = imageFormat;
		img1.flags = CL_MEM_WRITE_ONLY;
		img1.image_width = width;
		img1.image_height = height;

		img1.host_ptr = outputImageData2D;


		_ArgFirst(Args, CL_ARG_IMAGE | CL_ARG_POPULATE | CL_MEM_READ_ONLY,ClImageArg,img0, sizeof(ClImageArg));
    /* upward Pseudoprobability */
	   _ArgLast(Args,CL_ARG_IMAGE | CL_ARG_POPULATE | CL_MEM_WRITE_ONLY,ClImageArg,img1, sizeof(ClImageArg));
    /* local memory */
//	_ArgLast(Args,CL_ARG_LCL_MEM_SZ,int,GROUP_SIZE * 4 * sizeof(float),sizeof(int));
    ...
    * Enqueue a kernel run call.
    */
    int globalThreads[] = {width, height};
    int localThreads[] = {blockSizeX, blockSizeY};


	status = callCL(deviceType.c_str(), 2, globalThreads, localThreads, DEFAULT_KERNEL_LOCATION, "SimpleImage_Kernels.cpp", "image2dCopy",  Args);
